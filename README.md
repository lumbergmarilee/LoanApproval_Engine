...
It will find a client's credit_score and based on that it will decide whether or not to approve the loan. (btw credit_score = (credit_modifier/loan_amount)*loan_period )
...
Python is the language I am most comfortable with for creating frontend and using API (in this case Flask).
...
For the actual logic of the loan approval engine I decided to create just one python script with 5 functions. Didn't feel the need to create multiple files for this.
...
Now I had to add logic to the .html page. For getting information from the page (person ID, requested loan amount and requested loan period) and submitting an answer I created a **submitLoan** function. It first clears any previous results and reads the personal code, loan amount and loan period from the input fields. If the personal code is empty it will show an error.
It then disables the submit button and changes its text to "Checking…" so the user knows something is happening - this "Checking" rarely actually shows since I have 4 hard coded values for ID's and the code runs pretty fast. 
...
For the API I created a single Flask application in app.py. It uses os.path.abspath(__file__) to find the folder where the script lives, so Flask knows to look for the HTML, CSS and JS files in the same directory.

The app has two routes. 

The first one just serves the index.html page when someone visits localhost:5000 in their browser.

The second route is the actual API endpoint at /api/loan-decision which accepts POST requests. When the frontend submits a loan check, this endpoint reads the personal code, loan amount and loan period from the JSON body. 
I first made it do some validation. 
It checks that all three fields are present and that they can be converted to the right types (integer for personal code and period, float for amount). I decided not to validate the loan period range here because the engine will try to find a suitable period anyways if the requested one is out of bounds.
If validation passes it calls the loan_approval_algorithm from the engine and then formats the response. If the loan was approved it includes the approved amount and period. If the approved period or amount differs from what was requested it adds a note explaining the adjustment. If the loan was declined it includes the reason why. 


The response is returned as JSON which the frontend JavaScript then reads to build the result box.
